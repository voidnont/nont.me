...
import { loadFrxeApp, loadRepositoryApp } from '../server/github.js';
...
export default async function handler(req, res) {
  ...
  if (app && app !== 'frxe') return res.status(400).json({ error: 'Unsupported logical app.' });
  ...
  try {
    const data = app === 'frxe' ? await loadFrxeApp() : await loadRepositoryApp(repo);
