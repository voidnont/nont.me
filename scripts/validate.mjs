...
const cobaltKeyApi = read('api/cobalt-key.js');
const cobaltCookie = read('src/server/cobaltKeyCookie.js');
const cobaltShared = read('src/shared/cobalt.js');
const cobaltUnit = read('tests/unit/cobalt.test.mjs');
const cobaltKeyUnit = read('tests/unit/cobaltKeyCookie.test.mjs');
...
expect(cobaltApi.includes('COBALT_COOKIE_SECRET'), 'Cobalt authentication must use the encrypted browser key cookie');
expect(!cobaltApi.includes('process.env.COBALT_API_KEY'), 'Cobalt bridge must not use a shared global API key');
expect(cobaltKeyApi.includes("frxe_cobalt_key") || cobaltKeyApi.includes('COBALT_COOKIE_NAME'), 'Cobalt key API must manage the per-browser cookie');
expect(cobaltCookie.includes("'aes-256-gcm'"), 'Cobalt browser key must be encrypted with AES-256-GCM');
expect(cobaltCookie.includes("'HttpOnly'"), 'Cobalt browser key cookie must be HttpOnly');
expect(cobaltCookie.includes("'SameSite=Lax'"), 'Cobalt browser key cookie must use SameSite=Lax');
expect(cobaltKeyUnit.includes('encrypts without plaintext'), 'unit suite must cover encrypted Cobalt browser storage');
