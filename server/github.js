...
export async function loadFrxeApp(fetchImpl = fetch) {
  const settled = await Promise.allSettled(FRXE_APP.sources.map((source) => loadRepositoryApp(source.repo, fetchImpl)));
  const sources = settled.map((result, index) => {
    const source = FRXE_APP.sources[index];
  ...
  const merged = mergeFrxeSources(sources);
  return {
    ...merged,
    description: sources.find((source) => source.description)?.description || 'Frxe music player for desktop and mobile.',
    url: 'https://github.com/voidnont/frxe',
  };
}
