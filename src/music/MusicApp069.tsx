...
export const FRXE_WEB_VERSION = '0.6.9';
...
const SETTINGS_KEY = 'frxe.web.player.v1';
...
function loadLibrary() {
  ...
  return current.length ? current : loadTracks(LEGACY_LIBRARY_KEY);
}
...
export default function MusicApp069() {
  const initialSettings = useRef(safeJson(localStorage.getItem(SETTINGS_KEY), { volume: 76, muted: false, shuffle: false, repeat: 'off' }));
  ...
  const [history, setHistory] = useState<Track[]>(() => loadTracks(HISTORY_KEY));
  const [recentSearches, setRecentSearches] = useState<string[]>(() => loadStrings(RECENT_SEARCHES_KEY));
  ...
  useEffect(() => { try { localStorage.setItem(SETTINGS_KEY, JSON.stringify({ volume, muted, shuffle, repeat })); } catch {} }, [volume, muted, shuffle, repeat]);

  useEffect(() => {
    const win = window as any;
    if (!win.YT?.Player || playerRef.current) return;
    playerRef.current = new win.YT.Player('frxe-youtube-player', {
      events: {
        ...
        onStateChange: (event: any) => {
          ...
          if (event.data === win.YT.PlayerState.ENDED) { setPlaying(false); endedRef.current(); }
        },
        onError: (event: any) => {
          setPlaying(false);
  ...
  useEffect(() => {
    ...
    if (current && 'MediaMetadata' in window) {
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        album: `FRXE Web ${FRXE_WEB_VERSION}`,
  ...
  async function saveMedia(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/media-extract', {
        ...
        body: JSON.stringify({ url: saveUrl.trim(), downloadMode: saveMode, audioFormat: saveMode === 'audio' ? 'mp3' : 'best', audioBitrate: '320', videoQuality: '1080' }),
      });
      const data = await response.json().catch(() => ({}));
  ...
  return (
    <div className="frxe-app frxe069-app">
      ...
      <main className="frxe-stage frxe069-stage">
        ...
        {tab === 'settings' && (
          <section className="frxe-screen"><header className="frxe-heading compact"><div><h2>Settings</h2><p>Playback and release information.</p></div></header><div className="frxe-glass strong frxe069-settings"><label><span>Volume <b>{muted ? 0 : volume}%</b></span><input type="range" min="0" max="100" value={volume} onChange={(event) => setVolume(Number(event.target.value))} /></label><button className="frxe069-setting-row" onClick={() => setMuted((value) => !value)}>{muted ? <VolumeX size={18} /> : <Volume2 size={18} />} {muted ? 'Unmute' : 'Mute'}</button><button className="frxe069-setting-row" onClick={() => setShuffle((value) => !value)}><Shuffle size={18} /> Shuffle: {shuffle ? 'On' : 'Off'}</button><button className="frxe069-setting-row" onClick={() => setRepeat((value) => value === 'off' ? 'queue' : value === 'queue' ? 'track' : 'off')}><Repeat2 size={18} /> Repeat: {repeat}</button><div className="frxe069-version"><span>FRXE WEB</span><strong>v{FRXE_WEB_VERSION}</strong><small>Web release version is separate from Android source sync.</small></div></div></section>
        )}
      </main>
...
function EmptyState({ onSearch }: { onSearch: () => void }) {
  return <div className="frxe-glass strong frxe-empty-home"><Music2 size={34} /><h2>Build your sound</h2><p>Start searching and playing music. FRXE will mix artists, genres, favorites and new discoveries here.</p><button className="frxe-primary" onClick={onSearch}><SearchIcon size={17} /> Search music</button></div>;
}
