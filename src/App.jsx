...
const VERSION = '0.7.1';
const NEXUS_REPO = 'voidnont/NontHub';
const NONT_REPO = 'voidnont/NontMusic';
const NEXUS_LOGO = 'https://raw.githubusercontent.com/voidnont/NontHub/main/public/nonthub-logo.png';
...
const catalog = [
  { id: 'nontmusic', name: 'NontMusic', subtitle: 'Music, connected.', description: 'The current NontMusic Windows player and the music side of the NONT ecosystem.', category: 'MUSIC', kind: 'github', repo: NONT_REPO, extensions: ['.exe', '.msi'], featured: true, icon: Music2, iconUrl: NONT_LOGO },
  { id: 'nont-music-web', name: 'NontMusic Web', subtitle: 'Play in your browser.', description: 'The focused NontMusic web player at music.nont.me.', category: 'MUSIC', kind: 'web', route: 'https://music.nont.me', featured: true, icon: Music2, iconUrl: NONT_LOGO },
  { id: 'nonthub', name: 'NontHub', subtitle: 'Your NONT apps in one place.', description: 'The Windows app hub for releases, downloads and app management.', category: 'HUB', kind: 'github', repo: NEXUS_REPO, extensions: ['.msi', '.exe'], featured: true, icon: Boxes, iconUrl: NEXUS_LOGO },
  { id: 'web-search', name: 'Web Search', subtitle: 'Search without leaving NontHub.', description: 'Search the public web, inspect results and open sources.', category: 'WEB', kind: 'builtin', route: 'web-search', icon: Globe2 },
  { id: 'downloads', name: 'Downloads', subtitle: 'Release files in one place.', description: 'Open direct downloads and GitHub Release assets from NontHub.', category: 'TOOLS', kind: 'builtin', route: 'downloads', icon: Download },
];
...
export default function App() {
  ...
  return <div className="shell">
    <aside className="sidebar">
      <button className="brand" onClick={() => setPage('home')}><span className="brand-mark"><img src={NEXUS_LOGO} alt="NontHub" /></span><span><strong>NONT</strong><small>HUB</small></span></button>
      ...
      <div className="sidebar-bottom"><div className="mini"><span><img className="mini-app-icon" src={NONT_LOGO} alt="NontMusic"/></span><div><strong>NontMusic</strong><small>Web player ready</small></div><ChevronRight size={16}/></div><div className="status"><i/> NontHub online</div></div>
    </aside>

    <main className="main">
      <header className="topbar"><div className="search"><Search size={17}/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search NontHub library"/></div><div className="top-actions"><button className="icon-button" onClick={() => setTheme(theme === 'bright' ? 'dark' : 'bright')}>{theme === 'bright' ? <Moon size={17}/> : <Sun size={17}/>}</button><button className="primary compact" onClick={() => setShowAdd(true)}><Plus size={16}/> Add download</button></div></header>

      {page === 'home' && <section className="content">
        <div className="hero"><div><div className="eyebrow"><i/> NONT ECOSYSTEM · v{VERSION}</div><h1>Your apps.<br/><em>One signal.</em></h1><p>NontHub and NontMusic, connected through one responsive NONT home for releases, downloads and browser playback.</p><div className="hero-actions"><button className="primary" onClick={() => setPage('library')}><Boxes size={18}/> Open library</button><button className="secondary" onClick={() => { window.location.href = 'https://music.nont.me'; }}><img className="button-app-icon" src={NONT_LOGO} alt=""/> Open NontMusic Web</button></div></div><div className="signal"><span/><span/><div className="signal-core"><img src={NEXUS_LOGO} alt="NontHub"/></div><div className="bars">{Array.from({length:21}).map((_,i)=><i key={i} style={{height:`${18+((i*17)%48)}px`}}/> )}</div></div></div>
        <div className="home-split"><div><Title title="Featured" subtitle="Current NONT apps and web tools"/><div className="app-grid">{visibleApps.filter((a)=>a.featured).map((app)=><AppCard key={app.id} app={app} busy={releaseChecks[app.repo]?.status==='checking'} onOpen={()=>installApp(app)}/>)}</div></div><aside className="pulse"><span className="category">SYSTEM</span><h3>NONT pulse</h3><Pulse icon={<img className="pulse-app-icon" src={NEXUS_LOGO} alt=""/>} label="NontHub" value="Windows app hub"/><Pulse icon={<img className="pulse-app-icon" src={NONT_LOGO} alt=""/>} label="NontMusic" value="Desktop + web player"/><Pulse icon={<Download size={16}/>} label="Downloads" value={`${downloads.length} in history`}/><button className="ghost" onClick={()=>setPage('updates')}>Check ecosystem updates <ChevronRight size={14}/></button></aside></div>
      </section>}

      {page === 'library' && <section className="content"><Title title="Library" subtitle="NontHub, NontMusic and browser tools in one place."/><div className="app-grid all">{visibleApps.map((app)=><AppCard key={app.id} app={app} busy={releaseChecks[app.repo]?.status==='checking'} onOpen={()=>installApp(app)}/>)}</div></section>}
...
function AppCard({ app, onOpen, busy }) { const Icon=app.icon; const actionLabel = app.kind === 'web' ? 'Open' : app.kind === 'builtin' ? 'Open' : 'Download'; return <article className="app-card"><div className="app-card-top"><span className="app-icon">{app.iconUrl ? <img className="real-app-icon" src={app.iconUrl} alt={`${app.name} icon`}/> : <Icon size={27}/>}</span><span className="version-chip">v{VERSION}</span></div><div className="app-copy"><span className="category">{app.category}</span><h3>{app.name}</h3><strong>{app.subtitle}</strong><p>{app.description}</p></div><button className="secondary full" onClick={onOpen} disabled={busy}>{busy?<LoaderCircle className="spin" size={16}/>:app.kind==='builtin'?<ChevronRight size={16}/>:<PackageOpen size={16}/>} {busy?'Resolving…':actionLabel}</button></article>; }
