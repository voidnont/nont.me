...
const catalog = [
  ...
  { id: 'nont-music-web', name: 'NONT Music Web', subtitle: 'Play in your browser.', description: 'The focused NONT music player at music.nont.me.', category: 'MUSIC', kind: 'web', route: 'https://music.nont.me', featured: true, icon: Music2 },
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {page === 'home' && <section className="content">
        <div className="hero"><div><div className="eyebrow"><i/> NONT ECOSYSTEM · WEB</div><h1>Your apps.<br/><em>One signal.</em></h1><p>NONT Nexus, adapted for the browser: releases, downloads and web search in one responsive home.</p><div className="hero-actions"><button className="primary" onClick={() => setPage('library')}><Boxes size={18}/> Open library</button><button className="secondary" onClick={() => { window.location.href = 'https://music.nont.me'; }}><Music2 size={18}/> Open Web Player</button></div></div><div className="signal"><span/><span/><div className="signal-core"><img src={LOGO} alt=""/></div><div className="bars">{Array.from({length:21}).map((_,i)=><i key={i} style={{height:`${18+((i*17)%48)}px`}}/> )}</div></div></div>
...
function AppCard({ app, onOpen, busy }) { const Icon=app.icon; const actionLabel = app.kind === 'web' ? 'Open' : app.kind === 'builtin' ? 'Open' : 'Download'; return <article className="app-card"><div className="app-card-top"><span className="app-icon"><Icon size={27}/></span><span className="version-chip">v{VERSION}</span></div><div className="app-copy"><span className="category">{app.category}</span><h3>{app.name}</h3><strong>{app.subtitle}</strong><p>{app.description}</p></div><button className="secondary full" onClick={onOpen} disabled={busy}>{busy?<LoaderCircle className="spin" size={16}/>:app.kind==='builtin'?<ChevronRight size={16}/>:<PackageOpen size={16}/>} {busy?'Resolving…':actionLabel}</button></article>; }
