...
const NEXUS_LOGO = 'https://raw.githubusercontent.com/voidnont/NONT-Nexus/main/public/nexus-logo.png';
...
const catalog = [
  { id: 'nont', name: 'NONT', subtitle: 'Music, connected.', description: 'The NONT music player and the center of the NONT ecosystem.', category: 'MUSIC', kind: 'github', repo: NONT_REPO, extensions: ['.exe'], featured: true, icon: Music2, iconUrl: NONT_LOGO },
  { id: 'nont-music-web', name: 'NONT Music Web', subtitle: 'Play in your browser.', description: 'The focused NONT music player at music.nont.me.', category: 'MUSIC', kind: 'web', route: 'https://music.nont.me', featured: true, icon: Music2, iconUrl: NONT_LOGO },
...
export default function App() {
  ...
  return <div className="shell">
    <aside className="sidebar">
      <button className="brand" onClick={() => setPage('home')}><span className="brand-mark"><img src={NEXUS_LOGO} alt="NONT Nexus" /></span><span><strong>NONT</strong><small>NEXUS</small></span></button>
      ...
      <div className="sidebar-bottom"><div className="mini"><span><img className="mini-app-icon" src={NONT_LOGO} alt="NONT"/></span><div><strong>NONT</strong><small>Web download ready</small></div><ChevronRight size={16}/></div><div className="status"><i/> Nexus online</div></div>
    </aside>
    ...
    <main className="main">
      ...
      {page === 'home' && <section className="content">
        <div className="hero"><div><div className="eyebrow"><i/> NONT ECOSYSTEM · WEB</div><h1>Your apps.<br/><em>One signal.</em></h1><p>NONT Nexus, adapted for the browser: releases, downloads and web search in one responsive home.</p><div className="hero-actions"><button className="primary" onClick={() => setPage('library')}><Boxes size={18}/> Open library</button><button className="secondary" onClick={() => { window.location.href = 'https://music.nont.me'; }}><img className="button-app-icon" src={NONT_LOGO} alt=""/> Open Web Player</button></div></div><div className="signal"><span/><span/><div className="signal-core"><img src={NEXUS_LOGO} alt="NONT Nexus"/></div><div className="bars">{Array.from({length:21}).map((_,i)=><i key={i} style={{height:`${18+((i*17)%48)}px`}}/> )}</div></div></div>
        <div className="home-split"><div><Title title="Featured" subtitle="The first apps and tools inside your Nexus"/><div className="app-grid">{visibleApps.filter((a)=>a.featured).map((app)=><AppCard key={app.id} app={app} busy={releaseChecks[app.repo]?.status==='checking'} onOpen={()=>installApp(app)}/>)}</div></div><aside className="pulse"><span className="category">SYSTEM</span><h3>Nexus pulse</h3><Pulse icon={<img className="pulse-app-icon" src={NONT_LOGO} alt=""/>} label="NONT" value="Release download ready"/><Pulse icon={<Download size={16}/>} label="Downloads" value={`${downloads.length} in history`}/><Pulse icon={<Globe2 size={16}/>} label="Search" value="Serverless web search"/><button className="ghost" onClick={()=>setPage('updates')}>Check ecosystem updates <ChevronRight size={14}/></button></aside></div>
      </section>}
